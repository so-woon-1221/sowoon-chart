...
const LinearBarChart: ComponentType<ChartProps> = ({
  data,
  id,
  width,
  height,
  ...
  const keyList = useMemo(
    () =>
      Object.keys(data[0]).filter(
        k => k !== 'x' && k !== 'tooltipData' && k !== 'description',
      ),
    [data],
  );
  ...
  }, [colorScale, data, height, id, keyList, width, xScale, yScale]);
        ...
        onPointerMove={e => {
          if (tooltipMaker != null) {
            ...
            if (x != null) {
              ...
              if (data[index]) {
                const tooltipX = xScale(+data[index].x)!;
                const tooltipY =
                  groupType === 'single'
...
export default withParentSize(LinearBarChart);
